...
$flag=0;
 ...
 if (empty($_POST["name"])) {
  ...
  if (empty($_POST["roll"])) {
    $rollErr = "Teacher ID is required";
  }
   else {
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
  ...
  Teacher ID: <input type="text" name="roll" value="<?php echo $roll;?>">
...
if($flag==5)
{
      ...
      $sql="select id from teacher_profile where id='$roll'";
     ...
     else
     {
         ...
         $sql="insert into teacher_profile(name,id,contact,blood,dob) VALUES ('$name','$roll','$contact','$blood','$day1')";
          $reslt=$conn->query($sql);

          echo "Congratulations !!! . Profile Created Successfully<br>";

           
          $_SESSION["teacher_id"]=$roll;

           header("Location: /rmcenter_admin/rmc_teacher_image_upload.php");
           exit;
